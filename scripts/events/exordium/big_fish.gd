...
func _init() -> void:
	...
	super (EventType.IMAGE, NAME, DESCRIPTIONS[0], ImageMaster.event_img_big_fish)

	healAmt = int(CardGame.dungeon_main_screen.player.max_health / 3.0)
	options.append(OPTIONS[0] + str(healAmt) + OPTIONS[1])
	options.append(OPTIONS[2] + "5" + OPTIONS[3])
